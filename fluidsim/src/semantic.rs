...
use kinetics::{
    SemanticSnapshot, SemanticTile, TileFlags, SpeciesAmount, MaterialFraction,
    BoundarySummary, BoundaryFlags, SpeciesTableSnapshot, MaterialsTableSnapshot,
    SpeciesId as KineticsSpeciesId, MaterialId as KineticsMaterialId,
};
...
impl SemanticSnapshotBuilder {
    ...
    fn build_species_table(&self, registry: &SpeciesRegistry) -> SpeciesTableSnapshot {
        ...
        for info in registry.iter() {
            ...
            table.diffusion_coefficients.push(info.diffusion_coefficient);
        }
    ...
    fn aggregate_tile(
        ...
        for (mat_id, count) in material_counts {
            ...
            tile.material_fractions.push(MaterialFraction::new(
                KineticsMaterialId(mat_id),
                fraction,
            ));
        }
        ...
        if tile_x == 0 || tile_y == 0 || tile_x >= (fine_width / tile_size) - 1 || tile_y >= (fine_height / tile_size) - 1 {
    ...
    fn compute_global_stats(
        ...
        for (s, total) in global_totals.into_iter().enumerate() {
            if s < snapshot.species_table.species_ids.len() {
                ...
                snapshot.global_species_totals.push(SpeciesAmount::new(species_id, total));
            }
        }
        ...
        snapshot.global_thermal_energy = fluid_count as f64 * heat_capacity * snapshot.global_mean_temperature;
    }
    ...
    fn create_boundary(
        ...
        let is_interesting = 
            (tile_a.has_fluid() != tile_b.has_fluid()) ||
        ...
        // Temperature delta
        boundary.temperature_delta = tile_a.mean_temperature_kelvin - tile_b.mean_temperature_kelvin;

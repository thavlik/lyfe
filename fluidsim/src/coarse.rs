...
impl CoarseGrid {
    ...
    pub fn new(info: CoarseGridCreateInfo) -> Result<Self> {
        ...
        // Buffer sizes
        let coarse_conc_size =
            (species_count * coarse_cell_count * std::mem::size_of::<f32>()) as u64;
    ...
    pub fn record_compute(
        ...
        let descriptor_set =
            self.coarse_descriptor_sets[conc_current_buffer * 2 + temperature_current_buffer];
    ...
    pub fn request_cell_readback(&mut self, coarse_x: u32, coarse_y: u32) -> bool {
        ...
        unsafe {
            ...
            if self
                .device
                .begin_command_buffer(self.readback_command_buffer, &begin_info)
                .is_err()
            {
                return false;
            }
            ...
            for s in 0..self.species_count {
                let src_offset =
                    ((s * coarse_cell_count) + coarse_idx as usize) * std::mem::size_of::<f32>();
            ...
            let temperature_dst_offset =
                (self.species_count * std::mem::size_of::<f32>()) + std::mem::size_of::<u32>();
            ...
            if self
                .device
                .end_command_buffer(self.readback_command_buffer)
                .is_err()
            {
                return false;
            }
            ...
            if self
                .device
                .queue_submit(self.queue, &[submit_info], self.readback_fence)
                .is_err()
            {
                return false;
            }
...
impl Drop for CoarseGrid {
    fn drop(&mut self) {
        unsafe {
            ...
            self.device
                .destroy_command_pool(self.readback_command_pool, None);

            self.device.destroy_pipeline(self.coarse_pipeline, None);
            self.device
                .destroy_pipeline_layout(self.coarse_pipeline_layout, None);
            self.device
                .destroy_descriptor_pool(self.coarse_descriptor_pool, None);
            self.device
                .destroy_descriptor_set_layout(self.coarse_descriptor_set_layout, None);

            let mut alloc = self.allocator.lock();

            self.device
                .destroy_buffer(self.coarse_conc_buffer.buffer, None);
            alloc
                .free(std::mem::take(&mut self.coarse_conc_buffer.allocation))
                .ok();

            self.device
                .destroy_buffer(self.coarse_fluid_count_buffer.buffer, None);
            alloc
                .free(std::mem::take(
                    &mut self.coarse_fluid_count_buffer.allocation,
                ))
                .ok();

            self.device
                .destroy_buffer(self.coarse_temperature_buffer.buffer, None);
            alloc
                .free(std::mem::take(
                    &mut self.coarse_temperature_buffer.allocation,
                ))
                .ok();

            self.device
                .destroy_buffer(self.cell_readback_buffer.buffer, None);
            alloc
                .free(std::mem::take(&mut self.cell_readback_buffer.allocation))
                .ok();
        }
    }
}

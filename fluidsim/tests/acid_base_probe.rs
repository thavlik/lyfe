...
fn acid_base_center_window_heats_and_neutralizes() {
    ...
    assert!(output.status.success(), "probe failed: stdout={} stderr={}", String::from_utf8_lossy(&output.stdout), String::from_utf8_lossy(&output.stderr));
    ...
    assert!(final_h < initial_h, "expected H+ consumption near center: initial={initial_h}, final={final_h}");
    assert!(final_oh < initial_oh, "expected OH- consumption near center: initial={initial_oh}, final={final_oh}");
    assert!(final_temp > initial_temp, "expected exothermic heating near center: initial={initial_temp}, final={final_temp}");
}
